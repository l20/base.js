...
function Base(args) {
	...
	}else if (typeof args == 'function') {  //如果是函数执行DOM加载
		this.ready(args);
	}
}
...
//设置某一节点透明度
Base.prototype.opacity = function(target) {
	for (var i = 0; i < this.elements.length; i++) {
		this.elements[i].style.opacity = parseInt(target) / 100;
		this.elements[i].style.filter = 'alpha(opacity=' + parseInt(target) + ')';
	}
	return this;
}
...
Base.prototype.lock = function () {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		this.elements[i].style.display = 'block';
		parseFloat(sys.firfox) < 4 ? document.body.style.overflow = 'hidden' : document.documentElement.style.overflow = 'hidden';
		//addEvent(window, 'scroll', scrollTop);
		addEvent(this.elements[i], 'mousedown', prevdef); //chrome,firfox关闭选中文本默认行为，不可选中文本
		addEvent(this.elements[i], 'mouseup', prevdef);  	//取消默认的区域为this.elements如果是document的话登陆框的默认也会被取消
		addEvent(this.elements[i], 'selectstart', prevdef); //IE关闭选中文本默认行为，不可选中文本
		addEvent(window, 'scroll', fixedScroll);			//滚动条事件触发，不让登录窗口选中上面文本选中后往下拖
	}
	return this;
};
...
Base.prototype.unlock = function () {
	for (var i = 0; i < this.elements.length; i ++) {
		this.elements[i].style.display = 'none';
		parseFloat(sys.firfox) < 4 ? document.body.style.overflow = 'auto' : 
		document.documentElement.style.overflow = 'auto';
		removeEvent(this.elements[i], 'mousedown', prevdef); //chrome,firfox取消选中文本默认行为，可以选中文本
		removeEvent(this.elements[i], 'mouseup', prevdef);
		removeEvent(this.elements[i], 'selectstart', prevdef);  //IE取消选中文本默认行为，可以选中文本
...
Base.prototype.animate = function (obj) {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		if (start > target) step = -step;   //反方向运动
		...
		element.timer = setInterval(function () {  //使用定时器实现动画
